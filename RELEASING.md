...
3. Check the current version and release state.
   ...
   gh release list --repo muxinc/roku-mux --limit 5
   grep '"version"' package.json
   grep 'm.MUX_SDK_VERSION =' src/mux-analytics.brs
   ```
   Confirm the latest Git tag, the latest published GitHub release, and both
   version files all agree before bumping. Stop and report if any disagree.
...
6. Confirm both values match and the diff is clean.
   ```sh
   grep '"version"' package.json
   grep 'm.MUX_SDK_VERSION =' src/mux-analytics.brs
   git diff --check
...
1. Squash-merge the release PR. roku-mux uses squash merges; `master` history
   ...
   ```sh
   gh pr merge <PR_NUMBER> --repo muxinc/roku-mux --squash --delete-branch
   ```
...
2. Work in the Mux documentation site repository (it uses the `main` branch,
   not `master`). Use your local checkout of that repo, and create the docs
...
4. Decide whether feature docs need updates.
   ...
   - If no docs change is needed, report that decision and the reasoning.
...
9. After the docs PR is approved, squash-merge it and delete the branch.
   ```sh
